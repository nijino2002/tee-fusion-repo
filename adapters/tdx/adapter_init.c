...
static tee_status_t get_report(tee_buf_t* out){ if(!out) return TEE_EINVAL; const char* p=getenv("TDX_QUOTE_FILE"); if(!p){ out->ptr=NULL; out->len=0; return TEE_OK; }
...
static tee_status_t fill(const uint8_t* rpt,size_t n){ mapping_reset(); mapping_set_common("Intel-TDX","cvm",0,1); uint8_t m[32]={0}; mapping_set_sw_measurement(m,32); if(rpt&&n) mapping_set_native_quote(rpt,n); return TEE_OK; }
static tee_class_t pid(){ return TEE_CLASS_TDX; }
