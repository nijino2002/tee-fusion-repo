...
static tee_status_t get_report(tee_buf_t* out){ if(!out) return TEE_EINVAL; const char* p=getenv("KEYSTONE_REPORT_FILE"); if(!p){ out->ptr=NULL; out->len=0; return TEE_OK; }
...
static tee_status_t fill(const uint8_t* rpt,size_t n){ mapping_reset(); mapping_set_common("RISC-V-Keystone","enclave",0,1); uint8_t em[32]={0}; mapping_set_sw_measurement(em,32); uint8_t sm[32]={0}; mapping_add_extra_measurement(sm,32); if(rpt&&n) mapping_set_native_quote(rpt,n); return TEE_OK; }
static tee_class_t pid(){ return TEE_CLASS_KEYSTONE; }
